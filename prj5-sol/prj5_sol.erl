...
% 1 2 3 4 5 6 7 8  - passing  -30 testcases + 20 test cases + 23 
%   9 10 11 12 13 14  - failing
...
category_items1(_Category, []) ->
    [];
category_items1(Category, [Item | Rest]) when Item#order_item.category == Category ->
...
category_items2(Category, Items) ->
    lists:filter(fun (Item) -> Item#order_item.category == Category end, Items).
...
delete_item(Sku, Items) ->
    [Item || Item <- Items, Item#order_item.sku =/= Sku].
...
upsert_item(Item, [], Acc) ->
    lists:reverse(Acc) ++ [Item];
upsert_item(Item, [Item1 | Rest], Acc) when Item#order_item.sku == Item1#order_item.sku ->
    lists:reverse(Acc) ++ [Item | Rest];
upsert_item(Item, [Item1 | Rest], Acc) ->
    upsert_item(Item, Rest, [Item1 | Acc]).

    


% write the code for upsert_item/2 here
...
{ok, Result, Items};
_ ->
{err, lists:flatten(io_lib:format("item not found: ~p", [Sku])), Items}
end;
{upsert, Item} ->
{ok, void, upsert_item(Item, Items)};
_ ->
{err, lists:flatten(io_lib:format("bad Req ~p", [Req])), Items}
end.
...
start_items_server(Items, Fn) ->
    spawn(fun() -> items_server(Items, Fn) end).
...
stop_items_server() ->
    stop_items_server(items).

stop_items_server(Id) ->
    Pid = whereis(Id),
    Pid ! {self(), {stop}},
    receive
        {Pid, Response} -> Response
...
% Result from server.
% items_client(_Req) -> 
%   items_client(items, _Req).

% items_client(_Id, _Req) ->
%   items_client(items, _Req).
