...
(defn item-total [item]
  ...
  (* (:n-units item) (:unit-price item)))

(defn items-total1 [items]
  ;"return total price of all items"
  (reduce + (map item-total items)))
  ...
  (loop [items items
    ...
    (if (empty? items)
      ...
      (let [current-item (first items)]
        (recur (rest items)
               (+ total (* (:n-units current-item)
                            (:unit-price current-item))))))))
...
(defn item-category-n-units
  ...
  ([items] (reduce + (map :n-units items)))
  ([items category] (reduce + (map :n-units (filter #(= (:category %) category) items)))))