...
items_total2([order_item(_, _, Quantity, Price) | Rest], Acc, Total) :-
    NewAcc is Acc + Quantity * Price,
...
items_with_category([order_item(SKU, Category, Quantity, Price) | Rest], Category, [order_item(SKU, Category, Quantity, Price) | CategoryItems]) :-
...
expensive_item_skus([order_item(SKU, _, _, Price)|Rest], Threshold, ExpensiveSKUs) :-
    (Price > Threshold ->
        ExpensiveSKUs = [SKU|RestSKUs]
    ;
        ExpensiveSKUs = RestSKUs
    ),
    expensive_item_skus(Rest, Threshold, RestSKUs).
...
% #5: 15-points
% expensive_item_sku(Items, Price, SKU): SKU matches the sku of an order-item in Items having unit-price > price.
% Restriction: must be defined using a single rule, cannot use recursion.

expensive_item_sku(Items, Price, SKU) :-
    member(order_item(SKU, _, _, ItemPrice), Items),
    ItemPrice > Price.
...
left_plus(PlusExpr, LeftPlusExpr) :-
    left_plus(PlusExpr, [], LeftPlusExpr).
left_plus(Atomic, Acc, LeftPlusExpr) :-
    atomic(Atomic),
    !,
    (   Acc = []
    ->  LeftPlusExpr = Atomic
    ;   LeftPlusExpr = Acc + Atomic
    ).
left_plus(A + B, Acc, LeftPlusExpr) :-
    left_plus(A, Acc, LeftPlusA),
    left_plus(B, [], LeftPlusB),
    (   atomic(LeftPlusA)
    ->  (   atomic(LeftPlusB)
        ->  LeftPlusExpr = A + B
        ;   LeftPlusExpr = A + LeftPlusB
        )
    ;   (   atomic(LeftPlusB)
        ->  LeftPlusExpr = LeftPlusA + B
        ;   append(LeftPlusA, LeftPlusB, LeftPlusExpr)
        )
    ).
left_plus(A, Acc, LeftPlusExpr) :-
    A =.. [Op|Args],
    left_plus_list(Args, Acc, LeftPlusExpr).
left_plus_list([], Acc, Acc).
left_plus_list([H|T], Acc, LeftPlusExpr) :-
    left_plus(H, Acc, LeftPlusH),
    left_plus_list(T, LeftPlusH, LeftPlusExpr).
left_plus_list([H|T], LeftPlusExpr) :-
    left_plus(H, [], LeftPlusH),
    left_plus_list(T, LeftPlusH, LeftPlusExpr).
