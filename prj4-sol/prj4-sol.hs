...
instance Ord Term where
  ...
  compare (Var _) _ = LT
  compare _ (Var _) = GT
  compare (Struct n1 ts1) (Struct n2 ts2) =
...
itemsTotal items = sum $ map (\i -> fromIntegral (itemNUnits i) * itemUnitPrice i) items
...
sortBindings = sortBy (\(n1, _) (n2, _) -> compare n1 n2)
...
substTerm (Var varName) bindings =
  let maybeTerm = lookup varName bindings
  in fromMaybe (Var varName) $ fmap (\t -> substTerm t bindings) maybeTerm
substTerm (Struct atom terms) bindings =
  Struct atom $ map (\t -> substTerm t bindings) terms
...
normalizeBindings bindings =
  let
    sortedBindings = sortBy (comparing fst) bindings
    substBinding (varName, term) = (varName, substTerm term sortedBindings)
  in map substBinding sortedBindings
...
unify' (Var v1) (Var v2) bs
  | v1 == v2 = Just bs
  | otherwise = Just ((v1, Var v2):bs)
unify' (Var v) t bs = case lookup v bs of
  Just t' -> unify' t' t bs
  Nothing -> Just ((v, t):bs)
unify' t (Var v) bs = unify' (Var v) t bs
unify' (Struct n2 ts2) (Struct n1 ts1) bs
  | n2 /= n1 || length ts2 /= length ts1 = Nothing
  | otherwise = unifyList ts2 ts1 bs

-- solving for  unify: f(g(X), a) = f(g(Y), Y) => [(X, a), (Y, a)]
-- test case: 
...
-- -- code to pass the above test case
...
unifyList (t1:ts1) (t2:ts2) bs = case unify' t1 t2 bs of
  Just bs' -> unifyList ts1 ts2 bs'
  Nothing -> Nothing
unifyList _ _ _ = Nothing


--debugging for last test case

-- testUnify :: IO ()
-- testUnify = do
--   let t1 = Struct "f" [(Struct "g" [(Var "X")]), (Struct "a" [])]
--   let t2 = Struct "f" [(Struct "g" [(Var "Y")]), (Var "X")]
--   let expected = Just [("X", Struct "a" []), ("Y", Struct "a" [])]
--   let actual = unify t1 t2
--   putStrLn ("Expected: " ++ show expected)
--   putStrLn ("Actual: " ++ show actual)
--   assertEq "unify: f(g(X), a) = f(g(Y), Y) => [(X, a), (Y, a)]" actual expected
